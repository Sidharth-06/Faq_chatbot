...
export default function UpdatePasswordPage() {
  ...
  // Ensure user has an active session (i.e. redirected from reset link)
  useEffect(() => {
    async function checkSession() {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) {
        setError('No active session found. Please request a new password recovery link.');
      }
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Delay redirect to let user see success banner
      setTimeout(() => {
        router.push('/chat');
      }, 2000);
    } catch (err) {
  ...
  if (checkingSession) {
    return (
      <div className="min-h-screen bg-brand-cream text-zinc-950 flex items-center justify-center font-sans select-none relative overflow-hidden">
        ...
        <div className="flex flex-col items-center gap-3 relative z-10">
          ...
          <span className="text-zinc-950 text-xs font-black uppercase tracking-wider">Checking Session...</span>
        </div>
      </div>
    );
  }
        ...
        <SpotlightCard className="bg-white border-2 border-black p-8 shadow-[8px_8px_0px_0px_#000] rounded-none relative overflow-hidden transition-all duration-300 hover:shadow-[10px_10px_0px_0px_#000] w-full">
          ...
          {success && (
            ...
            >
              ...
              <span>Password updated successfully! Redirecting to chat...</span>
            </motion.div>
          )}
          ...
          {/* Fallback back to login if session check failed */}
          {error && !success && (
