 ...
 */
export async function searchWeb(query: string, limit: number = 3): Promise<FirecrawlSearchResult[]> {
  ...
  try {
    ...
    if (json.success && json.data?.web) {
      return json.data.web.map((item: any) => ({
        url: item.url || '',
 ...
 */
export async function scrapeWeb(url: string): Promise<string | null> {
  const apiKey = process.env.FIRECRAWL_API_KEY;
  if (!apiKey) {
    return null;
  }
 ...
 */
export async function getWebContext(message: string): Promise<WebContextResult | null> {
  ...
  const sources: FirecrawlSource[] = searchResults.map(res => ({
    title: res.title,
    url: res.url
  }));
