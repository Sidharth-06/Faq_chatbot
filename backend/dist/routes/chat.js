...
const conversation_context_1 = require("../lib/conversation-context");
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            if (attempt < maxRetries - 1) {
                ...
                console.warn(`Attempt ${attempt + 1} failed with status ${error.status}. Retrying in ${Math.round(delay)}ms...`);
                await sleep(delay);
            }
        }
    }
...
function processMessageAttachments(content) {
...
router.post('/', auth_1.requireAuth, async (req, res) => {
    ...
    try {
        ...
        const cleanedHistory = (history || []).map((msg) => {
            if (msg.role === 'user') {
                ...
                if (hasAttachments) {
                    try {
                        const attachments = JSON.parse(hasAttachments[1]);
        ...
        let webContext = null;
        try {
            webContext = await (0, firecrawl_1.getWebContext)(currentCleaned);
        }
        catch (e) {
    ...
    2. If describing workflows, sequential steps, architectural systems, or scraper cycles, you MUST output a standard Mermaid diagram block:
       ...
       Ensure the block starts with \`\`\`mermaid.`;
        ...
        const assembled = (0, conversation_context_1.assembleConversation)(systemInstruction, cleanedHistory, currentPrompt);
        const conversationMessages = (0, conversation_context_1.buildOpenAIMessages)(assembled);
        ...
        let fullText = '';
        let tokensUsed = 0;
        let reasoning_details = undefined;
        for await (const chunk of stream) {
            ...
            if (rawDelta?.reasoning_details) {
                ...
                if (typeof rawDelta.reasoning === 'string') {
                    if (typeof reasoning_details !== 'string')
                        reasoning_details = '';
